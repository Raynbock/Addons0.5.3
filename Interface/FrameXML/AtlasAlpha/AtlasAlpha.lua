...
function AtlasAlpha_OnLoad()
    -- Hide the default DialogBoxFrame OK button if it exists
    if ( this and this.GetName ) then
        local okButton = getglobal(this:GetName().."Button1");
        if ( okButton ) then
            okButton:Hide();
            okButton:Disable();
        end
    ...
    -- Increase info text size by ~3 points
    if ( AtlasAlphaInfoText and AtlasAlphaInfoText.SetTextHeight ) then
        AtlasAlphaInfoText:SetTextHeight(15);
    end
...
function AtlasAlpha_LootButton_OnClick()
    ...
    local entry = this.itemData;

    -- Internal hyperlink for tooltips / SetItemRef
    local hyperlink = AtlasAlpha_GetEntryLink(entry);
    if ( not hyperlink ) then
        return;
    end

    -- For chat, prefer the full stored hyperlink (|cff...|Hitem:...|h[Name]|h|r)
    local chatLink = hyperlink;
    if ( entry.link ) then
        chatLink = entry.link;
    end
